...
namespace Iir
{
    ...
    class fpmState<SampleType, CoefficientsType, DirectFormI>
    {
    public:
        fpmState() = default;

        fpmState(const DirectFormI &src) { set(src); }

        void set(const DirectFormI &src)
        {
            m_x2 = src.m_x2;
            m_y2 = src.m_y2;
            m_x1 = src.m_x1;
        ...
        {
            const SampleType out = s.m_b0 * in + s.m_b1 * m_x1 + s.m_b2 * m_x2 - s.m_a1 * m_y1 - s.m_a2 * m_y2;
            m_x2 = m_x1;
            m_y2 = m_y1;
            m_x1 = in;
            m_y1 = out;
    ...
    template <typename SampleType, typename CoefficientsType>
    class fpmState<SampleType, CoefficientsType, DirectFormII>
    {
    public:
        fpmState() = default;

        fpmState(const DirectFormII &src) { set(src); }

        void set(const DirectFormII &src)
        {
            m_v1 = src.m_v1;
            m_v2 = src.m_v2;
        }
    ...
    template <typename SampleType, typename CoefficientsType>
    class fpmState<SampleType, CoefficientsType, TransposedDirectFormII>
    {
    public:
        fpmState() = default;

        fpmState(const TransposedDirectFormII &src) { set(src); }

        void set(const TransposedDirectFormII &src)
        {
            m_s1 = src.m_s1;
            m_s1_1 = src.m_s1_1;
            m_s2 = src.m_s2;
    ...
    public:
        ...
        fpmRBJFilter(const RBJ::RBJbase &src)
        {
            set(src);
        }

        void set(const RBJ::RBJbase &src)
        {
            m_biquad.set(src);
        }
    ...
    private:
        fpmState<SampleType, CoefficientsType, DirectFormI> m_state;
